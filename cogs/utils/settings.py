...
default_path = "data/simbad/settings.json"
...
class Settings:
            ...
            if current.keys() != self.default_settings.keys():
                for key in self.default_settings.keys():
                    if key not in current.keys():
                        ...
                        print("Adding " + str(key) + " field to simbad settings.json")
    ...
    @property
    def default_recruiter(self):
        if "default" not in self.bot_settings:
            self.update_old_settings()
        return self.bot_settings["default"].get("RECRUITER_ROLE","")

    @default_recruiter.setter
    def default_recruiter(self, value):
        if "default" not in self.bot.settings:
            self.update_old_settings()
        self.bot_settings["default"]["RECRUITER_ROLE"] = value
        self.save_settings()
    ...
    def get_server_recruiter(self,server):
        if server is None:
            return self.default_recruiter
        assert isinstance(server,discord.Server)
        if server.id not in self.bot_settings:
            return self.default_recruiter
        return self.bot_settings[server.id].get("RECRUITER_ROLE","")

    def set_server_recruiter(self,server,value):
        if server is None:
            return
        assert isinstance(server,discord.Server)
        if server.id not in self.bot_settings:
            self.add_server(server.id)
        self.bot_settings[server.id]["RECRUITER_ROLE"] = value
        self.save_settings()
