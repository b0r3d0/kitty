...
description = """
Simbad Bot - Discord bot made by Leavism
"""
...
async def on_ready():
    ...
    if settings.login_type == "token":
        ...
        print(url)
        print("------")
    #await bot.get_cog('Mod').disable_commands()
...
def check_folders():
    folders = ("data", "data/simbad", "cogs", "cogs/utils")
...
def check_configs():
    if settings.bot_settings == settings.default_settings:
        print("\nSimbad Bot - First run configuration\n")
        print("A Discord python bot made by Leavism")
        ...
        if "@" not in choice and len(choice) >= 50:  # Assuming token
            ...
            os.remove('data/simbad/settings.json')
    ...
    if not os.path.isfile("data/simbad/cogs.json"):
        print("Creating new cogs.json...")
        dataIO.save_json("data/simbad/cogs.json", {})
...
def set_logger():
    ...
    handler = logging.FileHandler(
        filename='data/simbad/discord.log', encoding='utf-8', mode='a')
    ...
    logger = logging.getLogger("simbad")
    ...
    fhandler = logging.handlers.RotatingFileHandler(
        filename='data/simbad/simbad.log', encoding='utf-8', mode='a',
...
def set_cog(cog, value):
    data = dataIO.load_json("data/simbad/cogs.json")
    data[cog] = value
    dataIO.save_json("data/simbad/cogs.json", data)
...
def load_cogs():
    ...
    try:
        registry = dataIO.load_json("data/simbad/cogs.json")
    except:
        registry = {}
    ...
    if extensions:
        dataIO.save_json("data/simbad/cogs.json", registry)
...
if __name__ == '__main__':
    ...
    try:
        ...
        if choice.strip() == "reset":
            shutil.copy('data/simbad/settings.json',
                        'data/simbad/settings-{}.bak'.format(int(time.time())))
            os.remove('data/simbad/settings.json')
    except:
        logger.error(traceback.format_exc())
